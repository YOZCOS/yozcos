...
#undef TRACE_SYSTEM
#define TRACE_SYSTEM memcos

#if !defined(_TRACE_MEMCOS_H) || defined(TRACE_HEADER_MULTI_READ)
#define _TRACE_MEMCOS_H
...
TRACE_EVENT(access_info,
	    ...
	    TP_STRUCT__entry(__field(unsigned int, nr_access)
				     __field(unsigned int, nr_util)),

	    TP_fast_assign(__entry->nr_access = nr_access;
...
TRACE_EVENT(base_access_info,
	    ...
	    TP_STRUCT__entry(__field(unsigned long, addr) __field(unsigned int,
								  clock)
				     __field(unsigned int, nr_access)
					     __field(unsigned int, nr_util)),

	    TP_fast_assign(__entry->addr = addr; __entry->clock = clock;
			   __entry->nr_access = nr_access;
			   __entry->nr_util = nr_util;),

	    TP_printk("addr: %lu clock: %u nr_access: %u nr_util: %u\n",
		      __entry->addr, __entry->clock, __entry->nr_access,
		      __entry->nr_util));
