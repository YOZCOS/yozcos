					       ...
					       struct page *page, gfp_t gfp)
{
	...
	if (unlikely(!pmd_none(*vmf->pmd))) {
		...
		if (page != NULL && node_is_toptier(page_to_nid(page)))
			count_vm_events(MEMCOS_ALLOC_DRAM, HPAGE_PMD_NR);
		else
			count_vm_events(MEMCOS_ALLOC_NVM, HPAGE_PMD_NR);
#endif
	}
...
#ifdef CONFIG_MEMCOS
	prep_transhuge_page_for_memcos(vma, page);
#else
		 ...
		 unsigned long addr)
{
	...
	if (vma_is_special_huge(vma)) {
		...
		if (pmd_present(orig_pmd)) {
			page = pmd_page(orig_pmd);
#ifdef CONFIG_MEMCOS
			uncharge_memcos_page(
				page, get_mem_cgroup_from_mm(vma->vm_mm));
#endif
				    ...
				    unsigned long haddr, bool freeze)
{
	...
	if (PageMemcos(&page[3])) {
		...
		for (i = 0, addr = haddr; i < HPAGE_PMD_NR;
			...
			/* Memcos flag will be cleared later */
			/* ClearPageMemcos(&page[i]); */
		}
	}
skip_copy_pginfo:
#endif
				   ...
				   struct list_head *list)
{
...
#ifdef CONFIG_MEMCOS
	bool memcos_tail = PageMemcos(page_tail) ? true : false;
	bool memcos_active_tail = PageMemcos(head) && PageActive(page_tail);
#else
	bool memcos_tail = false;
	bool memcos_active_tail = false;
#endif
	...
	//VM_BUG_ON_PAGE(tail > 2 && !PageMemcos(head) && !memcos_tail && page_tail->mapping != TAIL_MAPPING,
	//			page_tail);

#ifdef CONFIG_MEMCOS
	if (memcos_tail)
		clear_transhuge_pginfo(page_tail);
	if (memcos_active_tail)
