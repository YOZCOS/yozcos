...
static int alloc_mem_cgroup_per_node_info(struct mem_cgroup *memcg, int node)
{
	...
	pn->on_tree = false;
	pn->memcg = memcg;
#ifdef CONFIG_MEMCOS /* alloc_mem_cgroup_per_node_info() */
...
static struct mem_cgroup *mem_cgroup_alloc(void)
{
...
#endif
#ifdef CONFIG_MEMCOS /* mem_cgroup_alloc() */
	memcg->memcos_enabled = false;
	...
	/* thresholds */
	memcg->active_threshold = memcos_thres_hot;
	memcg->warm_threshold = memcos_thres_hot;
	memcg->bp_active_threshold = memcos_thres_hot;
...
void __mem_cgroup_uncharge(struct page *page)
{
	...
	if (!page_memcg(page))
		return;
#ifdef CONFIG_MEMCOS
		//uncharge_memcos_page(page, page_memcg(page));
#endif
	uncharge_gather_clear(&ug);
...
#ifdef CONFIG_MEMCOS /* memcg interfaces for memcos */
static int memcg_memcos_show(struct seq_file *m, void *v)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(seq_css(m));

	if (memcg->memcos_enabled)
...
static ssize_t memcg_memcos_write(struct kernfs_open_file *of, char *buf,
				  size_t nbytes, loff_t off)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(of_css(of));
	int nid;
...
static struct cftype memcg_memcos_file[] = {
	{
		.name = "memcos_enabled",
		.flags = CFTYPE_NOT_ON_ROOT,
		.seq_show = memcg_memcos_show,
		.write = memcg_memcos_write,
	},
	{}, /* terminate */
};

static int __init mem_cgroup_memcos_init(void)
{
	WARN_ON(cgroup_add_dfl_cftypes(&memory_cgrp_subsys, memcg_memcos_file));
	return 0;
}
subsys_initcall(mem_cgroup_memcos_init);
...
static int memcg_mem_accesses_show(struct seq_file *m, void *v)
{
	...
	unsigned long long total_dram_accesses = 0, total_nvm_accesses = 0,
			   total_accesses = 0;
	...
	if (total_accesses > 0)
		perc_hit_rate =
			(int)(total_dram_accesses * 100 / total_accesses);

	seq_buf_printf(
		&s,
		"ACCESSES: dram %llu nvm or cxl %llu total %llu perc dram = %d \n",
		total_dram_accesses, total_nvm_accesses, total_accesses,
		perc_hit_rate);
...
static int pgdat_memcg_memcos_init(struct pglist_data *pgdat)
{
	pgdat->memcg_memcos_file =
		kzalloc(sizeof(struct cftype) * 2, GFP_KERNEL);
	if (!pgdat->memcg_memcos_file) {
		printk("error: fails to allocate pgdat->memcg_memcos_file\n");
		return -ENOMEM;
	}
#ifdef CONFIG_LOCKDEP
	lockdep_register_key(&(pgdat->memcg_memcos_file->lockdep_key));
#endif
	return 0;
}

int mem_cgroup_per_node_memcos_init(void)
{
	int nid;
	...
	for_each_node_state (nid, N_MEMORY) {
		...
		if (!pgdat || pgdat->memcg_memcos_file)
			continue;
		if (pgdat_memcg_memcos_init(pgdat))
			continue;

		snprintf(pgdat->memcg_memcos_file[0].name, MAX_CFTYPE_NAME,
			 "max_at_node%d", nid);
		pgdat->memcg_memcos_file[0].flags = CFTYPE_NOT_ON_ROOT;
		pgdat->memcg_memcos_file[0].seq_show = memcg_per_node_max_show;
		pgdat->memcg_memcos_file[0].write = memcg_per_node_max_write;
		pgdat->memcg_memcos_file[0].numa_node_id = nid;
...
subsys_initcall(mem_cgroup_per_node_memcos_init);
#endif /* CONFIG_MEMCOS */
