...
CGROUP_NAME="memcos"
###### update DIR!
DIR=/home/zly/memcos-v1/memcos-v1-userspace
...
function func_memtis_setting() {
    echo 199 | tee /sys/kernel/mm/memcos/memcos_sample_period
    echo 100007 | tee /sys/kernel/mm/memcos/memcos_inst_sample_period
    echo 1 | tee /sys/kernel/mm/memcos/memcos_thres_hot
    echo 2 | tee /sys/kernel/mm/memcos/memcos_split_period
    echo 100000 | tee /sys/kernel/mm/memcos/memcos_adaptation_period
    echo 2000000 | tee /sys/kernel/mm/memcos/memcos_cooling_period
    echo 4 | tee /sys/kernel/mm/memcos/memcos_mode
    echo 500 | tee /sys/kernel/mm/memcos/memcos_demotion_period_in_ms
    echo 500 | tee /sys/kernel/mm/memcos/memcos_promotion_period_in_ms
    echo 4 | tee /sys/kernel/mm/memcos/memcos_gamma
    ###  cpu cap (per mille) for ksampled
    echo 30 | tee /sys/kernel/mm/memcos/ksampled_soft_cpu_quota

    if [[ "x${CONFIG_NS}" == "xoff" ]]; then
	echo 1 | tee /sys/kernel/mm/memcos/memcos_thres_split
    else
	echo 0 | tee /sys/kernel/mm/memcos/memcos_thres_split
    fi

    if [[ "x${CONFIG_NW}" == "xoff" ]]; then
	echo 0 | tee /sys/kernel/mm/memcos/memcos_nowarm
    else
	echo 1 | tee /sys/kernel/mm/memcos/memcos_nowarm
    fi

    # if [[ "x${CONFIG_CXL_MODE}" == "xon" ]]; then
	# ${DIR}/scripts/set_uncore_freq.sh on
	# echo "enabled" | tee /sys/kernel/mm/memcos/memcos_cxl_mode
    # else
	# ${DIR}/scripts/set_uncore_freq.sh off
	echo "disabled" | tee /sys/kernel/mm/memcos/memcos_cxl_mode
    # fi
...
function func_main() {
    ...
    # set memcg for memcos
    # sudo ${DIR}/scripts/set_memcos_memcg.sh memcos remove
    sudo ${DIR}/scripts/set_memcos_memcg.sh memcos $$ enable
    sudo ${DIR}/scripts/set_mem_size.sh memcos 0 ${BENCH_DRAM}
    sleep 2
    ...
    cat /proc/vmstat | grep -e thp -e memcos -e pgmig > ${LOG_DIR}/before_vmstat.log 
    # flush cache
    func_cache_flush
    sleep 2
    ...
    cat /proc/vmstat | grep -e thp -e memcos -e pgmig > ${LOG_DIR}/after_vmstat.log
    sleep 2
    ...
    # disable memcos
    sudo ${DIR}/scripts/set_memcos_memcg.sh memcos $$ disable
}
