...
if [ "x$3" == "xenable" ]; then
    echo "enabled" | sudo tee ${CGROUP_DIR}/${CGROUP_NAME}/memory.memcos_enabled
    exit
elif [ "x$3" == "xdisable" ]; then
    echo "disabled" | sudo tee ${CGROUP_DIR}/${CGROUP_NAME}/memory.memcos_enabled
    exit
fi

echo "'set_memcos_memcg.sh' Invalid parameters...."
echo "./set_memcos_memcg.sh [cgroup name] [bash pid] [\"enable\" or \"disable\"]"
